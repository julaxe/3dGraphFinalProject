...
SpotLight sLight(glm::vec3(11.f, 50.0f, -11.0f),	// Position.
...
void init(void)
{
	...
	// Second texture. 
	unsigned char* image2 = stbi_load("Assets/center.jpg", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;
...
void Center(glm::vec2 Pos)
{
	glBindTexture(GL_TEXTURE_2D, secondTx);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.0f, 0.1f, 1.0f), X_AXIS, 0.0f, glm::vec3(Pos.x, -1.0f, Pos.y));
